...
interface DigitalProficiencyEventDao {
     ...
     * prevents permanently malformed digital-proficiency rows from cycling forever.
     *
     * Returns the number of rows that just transitioned to failed, so the caller
     * can log how many were given up on.
     */
    @Query(
        "UPDATE digital_proficiency_event SET sync_status = 'failed' WHERE id IN (:ids) AND retry_count >= :maxRetries",
    )
