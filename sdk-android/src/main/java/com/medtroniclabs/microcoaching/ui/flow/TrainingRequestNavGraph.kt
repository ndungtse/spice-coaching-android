...
/** Training-request routes (SK only), extracted from CoachingNavGraph (behaviour-preserving). */
internal fun NavGraphBuilder.trainingRequestGraph(
    navController: NavHostController,
    chwId: String,
    onFinish: () -> Unit,
    snackbarHostState: SnackbarHostState,
    docScope: CoroutineScope,
    context: Context,
) {
    // ── Training requests (SK only) ────────────────────────────────────────
    composable(CoachingRoute.TrainingRequests.route) {
        TrainingRequestsScreen(
            chwId = chwId,
            onNewRequest = {
                navController.whenSettled {
                    navController.navigate(CoachingRoute.TrainingRequestForm.route)
                }
            },
    ...
    composable(CoachingRoute.TrainingRequestForm.route) {
        TrainingRequestFormScreen(
            chwId = chwId,
            onSubmitted = {
                // Pop first so the snackbar shows on the hub behind it.
                navController.whenSettled { navController.popOrFinish(onFinish) }
                docScope.launch {
                    snackbarHostState.showSnackbar(
                        // Client-owned copy — a raised request is not the
                        // same as an available module (the backend's success
                        // message misleadingly says "available").
                        message = context.getString(R.string.training_request_submitted),
