...
fun ActiveSksScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().background(SurfaceMuted)) {
        SdkScreenHeader(title = stringResource(R.string.po_drilldown_active), onBack = onBack, onHome = onHome)
        when (val s = state) {
            ...
            is PODashboardUiState.Ready -> {
                ...
                Column(
                    ...
                    StatusGroup(R.string.po_group_active, SkStatus.ACTIVE, sks.filter { it.status == SkStatus.ACTIVE }, onOpenSkDetail)
                    StatusGroup(R.string.po_group_needs_attention, SkStatus.NEEDS_ATTENTION, sks.filter { it.status == SkStatus.NEEDS_ATTENTION }, onOpenSkDetail)
                    StatusGroup(R.string.po_group_inactive, SkStatus.INACTIVE, sks.filter { it.status == SkStatus.INACTIVE }, onOpenSkDetail)
